...
print'<html>';
print'<head> </head>';
print '<h2><font color="blue">HYDROGEN BONDING</font></h2>';
...
if($_=~/^ATOM/)
{
if((substr($_,17,3) eq "GLU") || (substr($_,17,3) eq "ASN") || (substr($_,17,3) eq "HIS") || (substr($_,17,3) eq "SER") || (substr($_,17,3) eq "THR") || (substr($_,17,3) eq "TYR") || (substr($_,17,3) eq "TRP") || (substr($_,17,3) eq "MET") || (substr($_,17,3) eq "CYS"))
{
if((substr($_,77,1) eq "H"))
{
push(@a,$_);
